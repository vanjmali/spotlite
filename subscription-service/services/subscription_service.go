...
func NewSubscriptionService(sr *repositories.SubscriptionRepository, gcc *adapters.GrpcContentChecker) *SubscriptionService {
...
func (s *SubscriptionService) Subscribe(req *dtos.CreateSubscriptionDto, ctx context.Context) error {
	...
	se, err := mappers.ToSubscriptionEntity(req, userIDstr)
	if err != nil {
		span.RecordError(err)
		return err
	}

	existenceCtx, existenceSpan := s.tr.Start(ctx, "subscription.create.entity_exists")
	defer existenceSpan.End()

	exists, err := s.gcc.CheckExistence(existenceCtx, req.EntityID, entities.SubscriptionType(req.Type))
	if err != nil {
		existenceSpan.RecordError(err)
		return err
	}
